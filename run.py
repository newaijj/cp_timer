...
def run_solution(
    solution_name, problem_maker_name, generator_input, verbose=False
):

    with open("generator_input.txt", "w") as g_i:
        g_i.write(str(generator_input))
        g_i.flush()
    ...
    with open("input.txt", "r") as f_i:
        with open("out.txt", "w") as f_o:
            time = timeit(
                stmt="subprocess.run( solution_path ,stdin = f_i, stdout = f_o)",
    ...
    if verbose:
        with open("out.txt", "r") as ans:
            info(
                "{} tested on {} with input {}".format(
                    solution_name, problem_maker_name, generator_input
                )
            )
